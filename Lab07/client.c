...
void run_client(int sockfd) {
    ...
    while (read(STDIN_FILENO, buf, BUFLEN - 1) > 0 && !isspace(buf[0])) {
        if (strlen(buf) == 1 && buf[0] == '\n') {
            // stop
            printf("[client] INFO: Empty message entered. Stopping client...\n");
            break;
        }
        ...
        DIE(bytes_send < 0, "[client] ERROR: Error while sending message to server\n");
        ...
        DIE(bytes_received < 0, "[client] ERROR: Error while receiving message from server\n");
...
int main(int argc, char* argv[])
{
    ...
    DIE(sockfd < 0, "[client] ERROR: Could not open a socket\n");
    printf("[client] INFO: Successfully opened a socket\n");
    ...
    int res = connect(sockfd, (struct sockaddr *) &serv_addr, socket_len);

    DIE(res < 0, "[client] ERROR: Could not connect to server\n");
